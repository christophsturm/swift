...
func swiftmutFindOrdinalExplicitConditionSourceLocation(
  moduleName: String,
  function: Function,
  ...
  guard let ordinal = swiftmutComparisonBranchOrdinalAndCount(
    function: function,
    branch: branch,
  ...
  for (path, preferredLine) in locatedSourcePaths {
    guard let result = swiftmutFindOrdinalExplicitConditionInFunctionBody(
      path: path,
...
func swiftmutComparisonBranchOrdinalAndCount(
  function: Function,
  branch: CondBranchInst,
  comparison: BuiltinInst
  ...
  for block in function.blocks {
    guard let candidateBranch = block.terminator as? CondBranchInst,
          let candidateComparison = candidateBranch.condition as? BuiltinInst,
          swiftmutComparisonBuiltinIDName(candidateComparison) == targetID else {
      continue
    }
    count += 1
  ...
  guard ordinal > 0,
        swiftmutCanUseComparisonOrdinalSourceLocation(count: count) else {
    return nil
  }
  return (ordinal, count)
}

func swiftmutFindOrdinalExplicitConditionInFunctionBody(
  path: String,
  preferredLine: Int,
  ordinal: Int,
  expectedCount: Int,
  ...
  func inspectLine(_ lineText: String, line: Int) {
    ...
    for expression in swiftmutGenericConditionExpressionCandidates(lineText) {
      guard swiftmutExplicitConditionExpressionMatchesMutation(
...
func swiftmutExplicitConditionExpressionMatchesMutation(
  _ sourceOriginal: String,
  mutation: SwiftmutMutation,
  config: SwiftmutConfig
) -> Bool {
  for rule in swiftmutSourceMutationDisplayRules(for: mutation, config: config) {
    guard !rule.sourceOriginal.isEmpty,
          rule.sourceOriginal != "condition" else {
      continue
    }
    if swiftmutReplaceFirstSourceOperator(
      rule.sourceOriginal,
      with: rule.sourceMutated,
      in: sourceOriginal
    ) != nil {
      return true
    }
  }
  return mutation.sourceOriginal == "condition"
}
