...
// RUN:   '}' > %t/config.json
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-swift-frontend -emit-sil -O -module-name SwiftmutConditionSourceScope -external-pass-pipeline-filename %t/pipeline.yaml %s -o /dev/null
// RUN: find %t/fragments -type f -name '*.json' -exec cat {} ';' > %t/all-fragments.json
// RUN: %FileCheck %s --input-file %t/all-fragments.json

public struct SwiftmutConditionSourceScope {
  ...
  public func grouped(_ site: Int, _ groups: [Int: Int]) -> Bool {
    if site == 1 {
      return groups[site] != nil
    }
    return false
  }
}
