...
// RUN:   '  ]' \
// RUN:   '}' > %t/config.json
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-swift-frontend -emit-sil -O -module-name SwiftmutGenericConditionSourceMutationText -external-pass-pipeline-filename %t/pipeline.yaml %s -o /dev/null
...
// CHECK-SAME: "sourceOriginal":"Set(selections.map(\\.siteID)).count >= 2","sourceMutated":"Set(selections.map(\\.siteID)).count < 2"
