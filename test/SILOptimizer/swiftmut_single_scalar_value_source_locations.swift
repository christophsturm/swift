...
// RUN:   '}' > %t/config.json
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-swift-frontend -emit-sil -O -module-name SwiftmutSingleScalarValueSourceLocations -external-pass-pipeline-filename %t/pipeline.yaml %s -o /dev/null
// RUN: find %t/fragments -type f -name '*.json' -exec cat {} ';' > %t/all-fragments.json
// RUN: %FileCheck %s --input-file %t/all-fragments.json
// RUN: %FileCheck %s --check-prefix=EVENTS --input-file %t/compiler-events.jsonl

public func swiftmutSinkScalarValue(_ value: Int) {
}

public func swiftmutSingleScalarValue(_ workers: Int) {
  let normalized = max(1, workers)
...
// EVENTS-SAME: "scalarValueSourceLocationMisses":"0"
