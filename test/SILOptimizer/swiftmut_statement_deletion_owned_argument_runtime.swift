...
// RUN:   '}' > %t/config.json
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-build-swift -O %t/main.swift -module-name SwiftmutStatementDeletionOwnedArgumentRuntime -Xfrontend -external-pass-pipeline-filename -Xfrontend %t/pipeline.yaml -o %t/a.out
// RUN: %target-codesign %t/a.out
// RUN: %target-run %t/a.out | %FileCheck %s
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-build-swift -O -D SWIFTMUT_BASELINE %t/main.swift -module-name SwiftmutStatementDeletionOwnedArgumentRuntime -Xfrontend -external-pass-pipeline-filename -Xfrontend %t/pipeline.yaml -o %t/baseline.out
...
// BASELINE: 2
// BASELINE-NEXT: 1
// EVENTS: "event":"metamutantDiscovery","module":"SwiftmutStatementDeletionOwnedArgumentRuntime","function":"{{.*}}swiftmutInvoke
...
// EVENTS: "event":"metamutantInjection","module":"SwiftmutStatementDeletionOwnedArgumentRuntime","function":"{{.*}}swiftmutInvoke
...
@discardableResult
@inline(never)
public func swiftmutConsumeAndReturn(_ token: consuming SwiftmutToken) -> Int {
...
public func swiftmutInvoke() {
  ...
  swiftmutConsumeAndReturn(token)
}
