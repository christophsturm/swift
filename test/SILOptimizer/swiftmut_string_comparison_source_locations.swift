...
// RUN:   '}' > %t/config.json
// RUN: env SWIFTMUT_CONFIG=%t/config.json %target-swift-frontend -emit-sil -O -module-name SwiftmutStringComparisonSourceLocations -external-pass-pipeline-filename %t/pipeline.yaml %s -o /dev/null
// RUN: find %t/fragments -type f -name '*.json' -print -exec %FileCheck %s --input-file {} ';'

public func swiftmutStringComparison(_ value: String) -> Bool {
  value == "paid"
}
